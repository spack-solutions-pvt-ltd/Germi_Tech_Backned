...
const employeeDetailIncludes = [
  { model: Role, as: "role" },
  {
    ...
    as: "documents",
    include: [{ model: Employee, as: "updater", attributes: ["id", "name", "level"] }],
  },
  {
...
async function getEmployeeById(req, res, next) {
  try {
    ...
    const employee = await Employee.findByPk(id, { include: employeeDetailIncludes });
    if (!employee) return error(res, 404, "Employee not found");

    const payload = employee.toJSON();
    payload.documents = payload.documents.map((d) => ({ ...d, displayName: documentDisplayName(d) }));
    payload.insurances = payload.insurances.map((i) => ({ ...i, displayType: insuranceDisplayType(i) }));

    return success(res, 200, "Employee fetched successfully", { data: payload });
  } catch (err) {
    next(err);
  }
}
...
async function createEmployee(req, res, next) {
  try {
    const { name, number, alternateNumber, email, level, roleId, status, joiningDate } = req.body;
    ...
    if (existingByEmail) return error(res, 409, "An employee with this email already exists");
    ...
    if (email) {
      try {
        await sendMail({
          ...
          html: welcomeEmailTemplate({
            ...
            loginUrl: process.env.FRONTEND_LOGIN_URL || "https://app.germitech.com/login",
          }),
        });
      } catch (mailErr) {
        emailSent = false;
      ...
      emailSent = false;
      emailError = "No email provided on this employee — share the password manually.";
    }

    return success(res, 201, "Employee created successfully", { emailSent, emailError });
  } catch (err) {
    next(err);
  }
}
...
async function updateEmployee(req, res, next) {
  try {
    ...
    if (number !== undefined && number !== employee.number) {
      const existingByNumber = await Employee.findOne({ where: { number, id: { [Op.ne]: id } } });
      if (existingByNumber) return error(res, 409, "An employee with this number already exists");
    }

    if (email !== undefined && email !== employee.email) {
      const existingByEmail = await Employee.findOne({ where: { email, id: { [Op.ne]: id } } });
      if (existingByEmail) return error(res, 409, "An employee with this email already exists");
    }
    ...
    return success(res, 200, "Employee updated successfully", { data: employee });
  } catch (err) {
    next(err);
  }
}
...
const updateEmployeeStatus = async (req, res, next) => {
  try {
    ...
    if (!["Active", "Inactive"].includes(status)) return error(res, 400, "Invalid status");
...
async function updateBankAccount(req, res, next) {
  try {
    ...
    if (!accountHolderName || !accountNumber || !ifscCode || !bankName) {
      return error(res, 400, "Account holder name, account number, IFSC code and bank name are required");
    }
    ...
    await employee.update({
      ...
      bankName,
      upiId,
      bankUpdatedBy: req.user?.id || null,
      bankUpdatedAt: new Date(),
    });

    const updated = await Employee.findByPk(id, {
      include: [{ model: Employee, as: "bankUpdater", attributes: ["id", "name", "level"] }],
    });

    return success(res, 200, "Bank account details updated successfully", { data: updated });
  } catch (err) {
    next(err);
  }
}
...
const DOCUMENT_TYPES = ["aadhaar_card", "pan_card", "drivers_license", "rc", "other"];
...
async function addEmployeeDocument(req, res, next) {
  try {
    ...
    if (!DOCUMENT_TYPES.includes(type)) {
      return error(res, 400, `type must be one of: ${DOCUMENT_TYPES.join(", ")}`);
    }
    if (type === "other" && !label) {
    ...
    return success(res, 201, "Document added successfully", {
      data: { ...document.toJSON(), displayName: documentDisplayName(document) },
    });
  } catch (err) {
    next(err);
  }
 ...
 * body: { type?, documentNumber?, label? }
 * The "Edit document" drawer — file re-upload optional, keeps the existing
 * file if none is chosen (matches the screenshot: it shows the current
 * filename and only swaps it if a new one is picked).
 */
async function updateEmployeeDocument(req, res, next) {
  try {
    const { id, documentId } = req.params;
    const document = await EmployeeDocument.findOne({ where: { id: documentId, employeeId: id } });
    ...
    const patch = {
      ...
      label: effectiveType === "other" ? label ?? document.label : DOCUMENT_LABELS[effectiveType],
      updatedBy: req.user?.id || null,
    };
 ...
 * "Download" icon. fileUrl already points at a static, publicly-servable
 * path (/uploads/employee-documents/...), so this only needs to redirect —
 * swap for res.download(absolutePath, originalFileName) if you'd rather
 * stream the file through the API instead of serving /uploads statically.
 */
async function downloadEmployeeDocument(req, res, next) {
  try {
    const { id, documentId } = req.params;
    const document = await EmployeeDocument.findOne({ where: { id: documentId, employeeId: id } });
    if (!document) return error(res, 404, "Document not found");
...
async function addEmployeeInsurance(req, res, next) {
  try {
    ...
    const { type, otherTypeName, provider, startDate, expiryDate, amount } = req.body;
    if (!INSURANCE_TYPES.includes(type)) {
      return error(res, 400, `type must be one of: ${INSURANCE_TYPES.join(", ")}`);
    }
    ...
    if (!provider || !startDate || !expiryDate || amount === undefined) {
      return error(res, 400, "provider, startDate, expiryDate and amount are required");
    }
    ...
    return success(res, 201, "Insurance added successfully", {
      data: { ...insurance.toJSON(), displayType: insuranceDisplayType(insurance) },
    });
  } catch (err) {
    next(err);
  }
...
async function updateEmployeeInsurance(req, res, next) {
  try {
    ...
    const insurance = await EmployeeInsurance.findOne({ where: { id: insuranceId, employeeId: id } });
    ...
    if (effectiveType === "other" && !(otherTypeName ?? insurance.otherTypeName)) {
      return error(res, 400, "otherTypeName is required when type is 'other'");
    }

    await insurance.update({
      ...(type !== undefined && { type }),
      otherTypeName: effectiveType === "other" ? otherTypeName ?? insurance.otherTypeName : null,
